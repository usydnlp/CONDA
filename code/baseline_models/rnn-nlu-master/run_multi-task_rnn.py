...
def get_perf(filename):
    ...
    ob = bytes(''.join(open(filename).readlines()),encoding="utf8")
    # stdout, _ = proc.communicate(''.join(open(filename).readlines()))
    stdout, _ = proc.communicate(ob)
      ...
      if current_step % FLAGS.steps_per_checkpoint == 0:
        ...
        # Run evals on development/test set and print the accuracy.
            ...
            if task['tagging'] == 1:
              ...
              sys.stdout.flush()
            print('hyp_label_list:',len(hyp_label_list))
            print('ref_label_list:',len(ref_label_list))
            print('ref_tag_list:',len(ref_tag_list))
            print('hyp_tag_list:',len(hyp_tag_list))
            return accuracy, tagging_eval_result, hyp_label_list, ref_label_list, hyp_tag_list, ref_tag_list
        ...
        test_accuracy, test_tagging_result,hyp_label_list,ref_label_list, hyp_tag_list,ref_tag_list = run_valid_test(test_set, 'Test') 
        pickle.dump(hyp_label_list, open( "./output_results/hyp_label_list"+str(current_step)+".pkl", "wb" ) )
        pickle.dump(ref_label_list, open( "./output_results/ref_label_list"+str(current_step)+".pkl", "wb" ) )
        pickle.dump(hyp_tag_list, open( "./output_results/hyp_tag_list"+str(current_step)+".pkl", "wb" ) )
        pickle.dump(ref_tag_list, open( "./output_results/ref_tag_list"+str(current_step)+".pkl", "wb" ) )
        if task['tagging'] == 1 \
